...
def update_iata():
    ...
    """
    sheet_data = dm.get_flight_sheet_data()  # Fetch current data from the sheet
...
def update_price():
    ...
    sheet_data = dm.get_flight_sheet_data()
    ...
    for row in sheet_data["prices"]:
        ...
        lowest_price, outbound_date, inbound_date, stoppage = fd.find_cheapest_flight(
        ...
        if stoppage is None:
            ...
            if lowest_price <= float(current_price):
                ...
                # Print details of the best deal found
                print(
                    f"🛫 Found a flight to {destination_iata} with a stoppage {stoppage} "
                    f"(Outbound: {new_flight_price_data[destination_iata]['outbound_date']}, "
                    f"Inbound: {new_flight_price_data[destination_iata]['inbound_date']}) "
                    f"→ Lowest round-trip price: ₹{new_flight_price_data[destination_iata]['price']} "
                    f"(Search window: {next_6month[0].strftime('%Y-%m-%d')} to {next_6month[-1].strftime('%Y-%m-%d')})"
    ...
    if "stoppage" in cheapest_data:
        message = (
            f"📢 Low price alert! Only ₹{cheapest_data['price']} to fly from {ORIGIN_LOCATION} to {cheapest_iata} with "
            f"stoppage {cheapest_data['stoppage']}"
...
users_data = dm.get_users_sheet_data()
# Loop through each user in the users sheet data
for user in users_data["users"]:
    # Unpack relevant details from each row
    _, first_name, last_name, email_address, id_ = user.values()
    nm.send_notification(fname=first_name, lname=last_name, email=email_address, update=notification)
