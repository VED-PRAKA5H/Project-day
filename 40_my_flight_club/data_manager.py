...
class DataManager:
    ...
    def __init__(self):
        ...
        self.prices_sheet_end_point = f"https://api.sheety.co/{self.sheet_id}/flightDeals/prices"
        # Construct the Sheety API endpoint URL for accessing the "users" sheet
        self.users_sheet_end_point = f"https://api.sheety.co/{self.sheet_id}/flightDeals/users"
    ...
    def update_cell(self, row_id, column, value):
        ...
        update_endpoint = f"{self.prices_sheet_end_point}/{row_id}"
    ...
    def get_flight_sheet_data(self):
        """
        ...
        response = requests.get(url=self.prices_sheet_end_point, headers=self.headers)
        try:
            # Try to parse and return the JSON response
    ...
    def get_users_sheet_data(self):
        """
        Fetch all data from the "users" sheet.
        ...
        """
        # Send a GET request to retrieve all rows from the sheet
        response = requests.get(url=self.users_sheet_end_point, headers=self.headers)
        try:
            # Try to parse and return the JSON response
            return response.json()
        except Exception as e:
