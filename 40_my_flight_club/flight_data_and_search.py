...
class FlightData:
    ...
    def search_flights(self, original_location, destination_location, from_date, to_date):
        ...
        # Return the list of flight offers from the response JSON if there are any direct flight
        if response.json()["data"]:
            ...
            body = {
                ...
                "returnDate": to_date,
                "adults": 1,  # Number of adult passengers
                "nonStop": "false",  # Only non-stop flights
                "currencyCode": "INR",  # Currency code for price
                "max": "10",  # Max number of flight offers to return
            }
    ...
    def find_cheapest_flight(self, date_range: list, origin_location_iata, destination_location_iata):
        ...
        - Tuple containing:
          (lowest_price (float),
           outbound_date (str in 'YYYY-MM-DD' format ),
           inbound_date (str in 'YYYY-MM-DD' format ),
           stoppage name if it's not direct else None)
        """
        ...
        for date in date_range:
            ...
            for flight in flights:
                ...
                # check if there is non-direct
                if len(flight['itineraries'][0]['segments']) != 1:
                    stoppage_iata_code = flight["itineraries"][0]['segments'][0]['arrival']['iataCode']
                    stoppages.append(stoppage_iata_code)
                else:
                    stoppages.append(None)
